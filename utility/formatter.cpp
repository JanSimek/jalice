...
public:
    ...
    std::string getElementName(XMLElement *element)
    {
    ...
    void Show()
    {
        ...
        if(!node)
        {
            std::cerr << "ERROR: not a valid AIML document" << std::endl;
            return;
        }
    ...
    void indent(int i)
    {
        std::cout << std::string(i, indenter);
    }
    ...
    void insertComment(std::string text)
    {
        ...
        // FIXME: this is wrong
        if(!elementstack.empty() && (elementstack.top().first == "template" ||
           elementstack.top().first == "category"))
        {
    ...
    {
        if(node == nullptr) return; // categories without "that"
...
int main(int argc, char *argv[])
{
    if(argc != 2)
    {
        std::cerr << "Usage: " << argv[0] << " file.aiml > formatted.aiml" << std::endl;
        return EXIT_FAILURE;
    }
