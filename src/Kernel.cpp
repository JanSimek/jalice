...
void Kernel::load(const string &file)
{
  ...
  if (loadedOk)
  {
    ...
    //        cout << "Learnt: " << file << " \t(" << (totalCategories - total)
    //        << " categories) \t(" << totalCategories << " total) \t(" <<
    //        (timerMillis() - time) << "ms)" << endl;
  }
  delete p;
}
...
void Kernel::convertTempAiml()
{
  ...
  PElement eCategory, ePattern, eThat, eTopic, eTemplate, eText;
...
void Kernel::loadTemporaryData()
{
  ...
  while (fin.good())
  {
    ...
    if (node != NULL)
    {
      node->addTemplate(new Template("temporary.data", startIndex, endIndex));
      ++totalCategories;
    }
  }
  fin.close();
 ...
 */
/*
Match *Kernel::match(Nodemaster *node, Nodemaster *parent, const string &input,
const string &star, const string &path) {
...
//        cout << "No _ (" << path << ")" << endl;
    ...
    if (node->getChild(word) != NULL) {
        if (word[0] == '<') {
            ...
            m = match(node->getChild(word), node, tail, star, path + " " +
word);
        }
        if (m != NULL) {
            if (word == "<that>") {
...
Match *Kernel::match(Nodemaster *node, Nodemaster *parent, int state, const string &input, const string &star, const string &path)
{
  ...
  if (node->getChild("@") != NULL)
  {
    ...
    // m = match(node.getChild("@" + type), node, state, tail, word, path + " @"
    // + type);
    // if (m != null) {
  ...
  if (node == parent->getChild("*") || node == parent->getChild("_") || node == parent->getChild("@"))
  {
...
string Kernel::process(Match *m, PElement e, Responder *r, const string &id) { return TemplateProcessor::processTemplate(m, e, r, id); }
string Kernel::respond(const string &input, const string &id) { return respond(input, id, 1, false); }
string Kernel::respond(const string &input, const string &id, bool srai) { return respond(input, id, 1, srai); }
string Kernel::respond(const string &input, const string &id, Responder *r) { return respond(input, id, r, 1, false); }
