  ...
  for (database::iterator it_d = d.begin(); it_d != d.end(); it_d++)
  {
    ...
    for (table::iterator it_t = t.begin(); it_t != t.end(); it_t++)
    {
      ...
      for (record::iterator it_r = r.begin(); it_r != r.end(); it_r++)
      {
        ...
        of << record_name << "," << id << "," << removeNewlines(r.data[0]) << "," << removeNewlines(r.data[1]) << ","
           << removeNewlines(r.data[2]) << "," << removeNewlines(r.data[3]) << "," << removeNewlines(r.data[4]) << ","
           << removeNewlines(r.data[5]) << "," << removeNewlines(r.data[6]) << endl;
      }
    }
    of.close();
  }
...
Record::Record() { data = new string[7]; }
Record::Record(const string &f1, const string &f2, const string &f3, const string &f4, const string &f5, const string &prop,
               const string &val)
{
  data = new string[7];
  data[0] = f1;
