...
PElement Parser::parse(istream &in, const string &)
{
  ...
  while (in.get(buf))
  {
    ...
    if ((buf == 'l' && buffer == "<?xml") || (buf == 'E' && buffer == "<!DOCTYPE"))
    {
    ...
    if (buf == '>' && buffer.find("<") == 0 && buffer.find(">") == buffer.length() - 1)
    {
    ...
    if (buf == '<' && buffer.length() > 1 && buffer.find("<") == buffer.length() - 1)
    {
...
PElement Parser::getRoot() const { return root->getChild(); }
void Parser::reset() { root = current = new Element("doc"); }
